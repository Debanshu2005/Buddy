...
def _scan_alsa_cards(tool: str) -> list[tuple[str, str, str]]:
    """
    Run `aplay -l` or `arecord -l` and return list of
    (card_number, card_name, device_number) for every USB card found.
    card_name is the short ALSA name (e.g. 'Device', 'USB_PnP_Sound_Device').
    """
    results = []
    try:
        ...
        for line in r.stdout.splitlines():
            low = line.lower()
            if line.startswith("card") and ("usb" in low or "uac" in low or "audio" in low):
                # "card 2: Device [USB Audio Device], device 0: USB Audio [USB Audio]"
                try:
                    card_num = line.split(":")[0].strip().split()[-1]
                    # Short name is between "card N: " and " ["
                    short = line.split(":")[1].strip().split("[")[0].strip()
                    # Normalise to ALSA-safe name (spaces → underscores)
...
def find_usb_audio_device() -> tuple[str, str]:
    """
    Auto-detect USB audio device for playback (aplay).
    Returns (aplay_device_str, card_index_str).

    Tries, in order:
      1. plughw:N,D  — most compatible for playback
      2. hw:N,D      — raw fallback
      3. default     — system default last resort
    """
    cards = _scan_alsa_cards("aplay")
    if cards:
        card_num, card_name, dev_num = cards[0]
        print(f"🔊 USB playback card detected: card {card_num} ({card_name}), device {dev_num}")
        return f"plughw:{card_num},{dev_num}", card_num

    print("⚠️ No preferred playback device found — defaulting to plughw:0,0")
    return "plughw:0,0", "0"
